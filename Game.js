...
export default class Game extends React.Component {
  ...
  //  const size = { width: 52, height: 120 };
  ...
  spawnPipe = async (openPos, flipped) => {
    ...
    if (flipped) {
      ...
      pipeY = Math.floor(openPos + OPENING / 2); // CURT CHANGE
    //  pipeY = Math.floor(openPos + OPENING / 2) - 80;
    }
      ...
      if (!this.gameOver) {
        ...
        this.pipes.forEachAlive(pipe => {
          ...
          if (pipeBox.intersectsBox(playerBox)) {
            console.log("player-y: " + this.player.y + " pipe-y: " + pipe.y + + " pipe-name: " + pipe.name);

            if(pipe.name == 'top' && ( (this.player.y + 195) > pipe.y) ){
              pipe.kill();
